...
class AllowSubcommand(private val sender: CommandSender, private val args: Array<String>, private val main: WorldManager):
    BukkitRunnable() {

    override fun run() {
        if (args.size < 3) {
            sender.sendMessage(Message.ALLOW_HELP.getMessage())
            return
        }
        ...
        if (args[2].contains("@")) {
            ...
            Flag.values().filter { inner -> inner.type == type}.forEach(profile::allowFlag)
            sender.sendMessage(
