...
class DenySpawnSubcommand(private val sender: CommandSender, private val args: Array<String>, private val main: WorldManager):
    BukkitRunnable() {

    override fun run() {
        if (args.size < 3) {
            sender.sendMessage(Message.DENY_SPAWN_HELP.getMessage())
            return
        }
        ...
        try {
            ...
            if (profile.denySpawn(spawnType)) {
                sender.sendMessage(Message.DENY_SPAWN_SUCCESS.getMessage()
                    .replace("%entity", spawnType.toString().lowercase())
                    .replace("%world", world.name))
            } else {
                sender.sendMessage(Message.DENY_SPAWN_ALREADY.getMessage()
                    .replace("%entity", spawnType.toString().lowercase())
                    .replace("%world", world.name))
            }
