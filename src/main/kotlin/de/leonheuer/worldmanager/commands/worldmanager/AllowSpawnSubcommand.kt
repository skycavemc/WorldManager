...
class AllowSpawnSubcommand(private val sender: CommandSender, private val args: Array<String>, private val main: WorldManager):
    BukkitRunnable() {

    override fun run() {
        if (args.size < 3) {
            sender.sendMessage(Message.ALLOW_SPAWN_HELP.getMessage())
            return
        }
        ...
        try {
            ...
            if (profile.allowSpawn(spawnType)) {
                sender.sendMessage(
